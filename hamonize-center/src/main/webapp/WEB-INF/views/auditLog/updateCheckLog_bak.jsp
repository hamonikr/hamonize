...
.monitor_list {height:500px; overflow:scroll; padding:20px 20px 30px 20px; width: 90%; overflow-x: hidden;}
	...
	$(".result_detail").remove();
	 $.post("detailPolicy.proc",{pc_uuid:uuid},
		...
		//if( data.udpt.length > 0){
			...
			shtml += "<caption>업데이트 배포 결과</caption>";
			shtml += "<thead><tr>";
			shtml += "<td>패키지</td>";
			shtml += "<td>버전</td>";
			shtml += "<td>구분</td>";
		...
		//if(data.program.length > 0){  
			...
			shtml += "<table class=\"result_detail\" border=\"1\">";
			shtml += "<caption>프로그램 차단 배포 결과</caption>";
			shtml += "<thead><tr>";
			shtml += "<td>패키지</td>";
			shtml += "<td>적용여부</td>";
			shtml += "<td>적용일</td>";
		...
		//}
			...
			console.log("data======"+data);
			shtml += "<table class=\"result_detail\" border=\"1\">";
			shtml += "<caption>방화벽 정책 배포 결과</caption>";
			shtml += "<thead><tr>";
			shtml += "<td>포트번호</td>";
			shtml += "<td>상태</td>";
			shtml += "<td>적용여부</td>";
		...
		//if(data.device.length > 0){  
			...
			console.log("data======"+data);
			shtml += "<table class=\"result_detail\" border=\"1\">";
			shtml += "<caption>디바이스 허용 배포 결과</caption>";
			shtml += "<thead><tr>";
			shtml += "<td>패키지</td>";
			shtml += "<td>적용여부</td>";
			shtml += "<td>적용일</td>";
			...
			// //console.log("data.nxss.length======"+data.nxss.length);
			// console.log("data======"+data);
			// shtml += "<table class=\"result_detail\" border=\"1\">";
			// shtml += "<caption>유해사이트 차단 배포 결과</caption>";
			// shtml += "<thead><tr>";
			// shtml += "<td>구분</td>";
			// shtml += "<td>적용여부</td>";
			// shtml += "<td>적용일</td>";
		...
		$.post("/mntrng/pcPolicyList",{org_seq:treeNode.id},
			...
			if( data.pcList.length > 0){
				...
				$.each(data.pcList, function(index, value) {
					 if(value.sgb_pc_status == "true"){
						cnt++;
					console.log(value.sgb_pc_status);
					shtml += "<li class='on'><a href='#' onclick=\"detail('"+value.sgb_pc_uuid+"')\">"+value.sgb_pc_hostname+"</a></li>";
						} else
					shtml += "<li><a href='#' onclick=\"detail('"+value.sgb_pc_uuid+"')\">"+value.sgb_pc_hostname+"</a></li>";
					 
				});	
				shtml_r += "<table class=\"result_detail\" border=\"1\">";
				shtml_r += "<caption>업데이트 배포 결과</caption>";
				shtml_r += "<thead><tr>";
				shtml_r += "<td>패키지</td>";
				shtml_r += "<td>버전</td>";
				shtml_r += "<td>구분</td>";
				...
				for(var i = 0;i < data.policyResult.length;i++){
				 ...
				 /* $.each(data.policyResult, function(index, value) {
					var inset_dt = value.ins_date;
					var date = new Date(inset_dt);
					date = date.getFullYear()+"-"+addZero(date.getMonth()+1)+"-"+addZero(date.getDate().toString());
					shtml_r += "<tr>";
					shtml_r += "<td>"+value.debname+"</td>";
					shtml_r += "<td>"+value.debver+"</td>";
					shtml_r += "<td>"+value.gubun+"</td>";
					shtml_r += "<td>"+data.pcList.length+"</td>";
					shtml_r += "<td>"+value.count+"</td>";
					shtml_r += "<td>"+date+"</td>";
					shtml_r += "</tr>";
				});	  */
				 shtml_r += "</tbody>";
	...
	if( data.length > 0 ){
		$.each(data, function(index, value) {
			...
			$(".cyberinfo li").each(function (i,v){
				...
				/*if(i==4)
					$(this).html("<span>기술지원 요청</span>"+sum1+"<br/>-<br/>"+sum1); */
...
<body>
    ...
    <div class="hamo_container other">
        ...
        <div class="con_right">
            <div class="right_box">
            <div class="right_box_l" style="float: left; width: 40%;"><h3>PC리스트</h3>
