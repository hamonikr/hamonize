...
$(document).ready(function() {
	...
	getList();
	  ...
	  /* $.each("#sm_seq", function(index, value) {
		value.on("change",function(){
			alert("asd");
			if($("#sm_seq").is(":checked")){
	            alert("체크박스 체크했음!");
	        }else{
	            alert("체크박스 체크 해제!");
	        }
	...
	//$("input:checkbox[name=chkFile]")
	  $("#sm_seq").bind("change",function(){
		  alert("as");
	  });
	 /* $("#sm_seq").change(function(){
		if($("#sm_seq").is(":checked")){
            alert("체크박스 체크했음!");
        }else{
            alert("체크박스 체크 해제!");
	...
	/* if($('.mdl-data-table__cell--non-numeric .form-control:checked')){
		$("#saveDevice").val("수정");
	} */
});
...
var deviceGetSuccess = function(data, status, xhr, groupId){
	...
	if( data.list.length > 0 ){
		$.each(data.list, function(index, value) {
			...
			//console.log(result[index].ppm_seq+"======"+value.sm_seq);
				if(value.ppm_seq == value.sm_seq){
					//gbInnerHtml += '<input type="checkbox" class="form-control" disabled></td>';
				}else{
					gbInnerHtml += '<input type="checkbox" id="sm_seq" class="form-control"></td>';	
				}
		...
		gbInnerHtml += "<tr><td colspan='3'>등록된 정보가 없습니다. </td></tr>";
	}
				...
				<div class="row" style="margin-left: 10px;">
					...
					<form id="addForm" class="form-inline col-md-12 row" action="">
			  			<div class="input-field s3 mb-4 mr-sm-4">
			            	...
			            	<label for="sm_name" class="mb-4 mr-sm-4">디바이스명</label>
			          	</div>
			          	
			  			<div class="input-field s3 mb-4 mr-sm-4">
						  	<input id="sm_dc" name="sm_dc" type="text" data-length="10">
			            	<label for="sm_dc" class="mb-4 mr-sm-4">디바이스정보</label>
			          	</div>
