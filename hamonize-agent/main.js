...
//	Firewall 정책 정보 조회 
	var setUrl = "http://"+sgbCenterUrl+"/getAgent/firewall?name="+uuid;
...
//	Recovery 정책 정보 조회
	var setUrl = "http://"+sgbCenterUrl+"/getAgent/recov?name="+uuid;
	http.get(setUrl, (res) => {
		res.on('data', (data) => {
...
function getUpdtDataCall(uuid){
	// log.info("updt job action");
//	Updt 정책 정보 조회
	
	var setUrl = "http://"+sgbCenterUrl+"/getAgent/updt?name="+uuid;
...
//	retData = "{\"UPS\":\"chrome=google-chrome\",\"INS\":\"libreoffice=libreoffice,\"}"
    ...
    //    exec(" sudo sh /usr/share/hamonize-agent/shell/updtjob.sh" , function (err, stdout, stderr) {
		exec(" sudo sh /home/lee/sgb-pc-agent/sgb-pc-agent/shell/updtjob.sh" , function (err, stdout, stderr) {	
...
//	Nxss 정책 정보 조회
	var setUrl = "http://"+sgbCenterUrl+"/getAgent/nxss?name="+uuid;
...
function sendNxssResultToCenter(fileDate, gubun){
	...
	log.info("fileDate------------" + fileDate+"=="+ gubun);
        ...
        request.post('http://'+sgbCenterUrl+'/act/nxssAct', {
                json: {
...
//	device 정책 정보 조회
	...
	var setUrl = "http://"+sgbCenterUrl+"/getAgent/device?name="+uuid;
...
function sendToCenter_result(product, vendorCode, prodcutCode, statusyn){
	...
	log.info("sgbCenterUrl : "+sgbCenterUrl);
	...
	request.post('http://'+sgbCenterUrl+'/act/deviceAct', {
		json: {
...
async function fnDeviceJob(retData){
	...
	var os = new os_func();
	os.execCommand("sudo /usr/local/bin/centor-lockdown").then(res=> {
...
//	progrm 정책 정보 조회
	var setUrl = "http://"+sgbCenterUrl+"/getAgent/progrm?name="+uuid;
	http.get(setUrl, (res) => {
		res.on('data', (data) => {
...
//	retData = "{\"INS\":\"hamonia,libreoffice\"}";
...
//	백업 정책 정보 조회
	var setUrl = "http://"+sgbCenterUrl+"/getAgent/backup?name="+uuid;
	http.get(setUrl, (res) => {
		res.on('data', (data) => {
