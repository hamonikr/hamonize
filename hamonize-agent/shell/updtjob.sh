...
do      
        ...
        echo "already installed deb file version is ====>$INSTALLED_VER" >>$LOGFILE 

        if [ -z $INSTALLED_VER ]; then
                echo "If this package is not installed, then istall..  ====>" >>$LOGFILE
                ...
                echo "This package is installed  ver is $INSTALLED_VER" >>$LOGFILE
                while [ `expr $INSTALLED_VER \!= $VERSION` = 1 ] 
                do
                        if [ `expr $INSTALLED_VER \< $VERSION` = 1 ]; then
                                echo "새버전이 더 크다 ===> $VERSION" >>$LOGFILE
                                ...
                                echo "upgrade chk wc -l is ==> $UPS_CHKUPS_PKG_VER_CNT" >>$LOGFILE
                                ...
                                if [ `expr $INSTALLED_VER \= $VERSION` = 1 ]; then
                                        echo "가져온 버전이랑 설치된 버전이랑 같다 ===> $INSTALLED_VER" >> $LOGFILE
                                ...
                                echo "설치된 버전이 더 크다 ===> $INSTALLED_VER" >>$LOGFILE
                        fi
