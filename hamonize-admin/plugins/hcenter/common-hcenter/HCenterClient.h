...
class HCenterClient : public QObject
{
	Q_OBJECT
public:
	enum class Scope {
    ...
    HCenterClient( const HCenterConfiguration& configuration, const QUrl& url = QUrl(), QObject* parent = nullptr );
    ~HCenterClient();

    const HCenterConfiguration& configuration() const
	{
