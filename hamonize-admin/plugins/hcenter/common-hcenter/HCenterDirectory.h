/*
 * HCenterDirectory.h - class representing the HCenter directory and providing access to directory entries
 *
...
class HCenterDirectory : public QObject
{
	Q_OBJECT
public:
    HCenterDirectory( const HCenterConfiguration& configuration, QObject* parent = nullptr );
    ~HCenterDirectory() = default;

	const QString& configInstanceId() const;

    const HCenterClient& client() const
	{
		return m_client;
	}

    HCenterClient& client()
	{
		return m_client;
	}
    ...
    QString hostToHCenterFormat( const QString& host );
...
private:
    HCenterClient::Scope computerSearchScope() const;

    const HCenterConfiguration& m_configuration;
    HCenterClient m_client;

    HCenterClient::Scope m_defaultSearchScope = HCenterClient::Scope::Base;
